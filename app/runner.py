...
Same design as the library runner; tools are now the two leave specialists."""
import time
...
def rebuild(store: RunStore, thread_id: str, run_id: str):
    """Rebuild what the model had seen from the database.
...
def execute_run(claimed: Claimed, *, store: RunStore, db: LeaveDb, providers: dict,
    ...
    while True:
        for step_seq, call in pending:
            ...
            if record_delay:
                time.sleep(record_delay)    # crash drill: side effect done, record not yet written
            ...
            if on_step:
                on_step({"agent": "supervisor", "run_id": run_id, "step": step_seq,
                         "kind": "tool", "tool": call["name"], "args": call["args"],
        ...
        if on_step:
            on_step({"agent": "supervisor", "run_id": run_id, "step": seq,
                     "kind": "model", "text": turn.text, "tool_calls": calls})
